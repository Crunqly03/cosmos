...
const IfoCardContribute: React.FC<Props> = ({
  ...
  const [liquidityIsCreated,setLiquidityIsCreated]=useState(false)
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const availabeToHarvest = await contract.methods.liquidityIsCreated().call()
      setLiquidityIsCreated(availabeToHarvest)
  ...
  }, [account,contract, contract.methods, pendingTx,userInfo,])
  ...
  const claim = async (harvestPeriod: number) => {
    if(!liquidityIsCreated){
      alert("Harvest will be available soon.Please be patient")
      return
    }
    setPendingTx(true)
